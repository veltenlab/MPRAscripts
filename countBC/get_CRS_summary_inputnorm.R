get_CRS_summary_inputnorm <- function(f, run_id,
                            use.col = "READS", #the column in the PERL pipeline output to use (e.g. UMI5: UMIs with >=5 reads)
                            plot =T, #create plots on the way in the directory specified as outdir
                            outdir = "allout/inputnormalize", 
                            normalize.to.input= T,
                            min.dna.umi = 0, #to filter at the level of CRS barcodes: Only include CRS BC covered with at least this many UMIs at DNA level
                            ...
                            bc_crs_ass = "/users/lvelten/project/SCG4SYN/Experiments/221118_LibraryA_recloned_association/Analysis/mapped_filter.csv.gz",
                            crs_filter =2500, #min UMIs per CRS to be flagged as pass
                            return_report = T, #besides returning the CRS data frame also return a list with some summary stats?
...
)
{
  ...
  #read in data (skipped if data is provided)
    ...
    #merge this with the BC-CRS-ASS experiment
    BC_CRS <- read.csv(bc_crs_ass)
    BC_CRS <- subset(BC_CRS, BARCODE %in% union(RNA[[1]]$BARCODE, RNA[[2]]$BARCODE)) #this was maybe not even so clever
    
    Experiments <- lapply(RNA, function(x) merge(x, BC_CRS, all=T, by = c("BARCODE","CRS"), suffixes = c(".rna",".dna") ))
  ...
  if(plot) {
    ...
    all.filtered <- subset(all, USE.dna.1 > 0 | USE.dna.2 > 0 )
    toplot<-sample(1:nrow(all.filtered), 50000)
    
    png(file.path(outdir, run_id, "BC_replicates_%02d.png"),width=800,height=600,res=100)
    heatscatter(log10(all.filtered$USE.dna.1[toplot]+1),log10( all.filtered$USE.rna.1[toplot]+1), main = "DNA vs RNA Exp 1", cor=T, method = "pearson")
    heatscatter(log10(all.filtered$USE.rna.1[toplot]+1),log10( all.filtered$USE.rna.2[toplot]+1), main = "RNA vs RNA", cor=T, method = "pearson")
    heatscatter(log10(all.filtered$USE.dna.1[toplot]+1),log10( all.filtered$USE.dna.2[toplot]+1), main = "DNA vs DNA", cor=T, method = "pearson")
    dev.off()
  }
  ...
  Experiments.filtered <- lapply(Experiments, function(x) 
    subset(x, umi.filter.connection(USE.dna >= min.dna.umi, USE.rna >= min.rna.umi)))
