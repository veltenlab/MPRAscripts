...
my ($bc, $bam, $outfile, $rc_dict_file) = @ARGV;
...
my $includes_rc_sequences = 0;
my %RCdict;
if (-e $rc_dict_file) {
  $includes_rc_sequences = 1;
  open(RC, "<","$rc_dict_file");
    while (my $rcline = <RC>) {
      chomp $rcline;
      my @rcsplit = split /\s+/, $rcline;
      $RCdict{$rcsplit[0]} = $rcsplit[1];
    }
    close(RC);
} else {
  warn "No (valid) RC dictionary file supplied, assuming that library contains no reverse complemented sequences."
}
...
sub cigar_to_score {
  ...
  my %maps_score;
  my %maps_count_fwd_1stread;
  my %maps_count_rev_1stread;
  my %maps_count_fwd_2ndread;
  my %maps_count_rev_2ndread;
  ...
  while ($bamline = <BAM>) {
    ...
    my @bits = reverse(split(//, sprintf("%010b", $bamsplit[1]))); #parse out flags
    
    if ($bamsplit[0] ne $readname) { #if the bam file is ahead
      #wrap up
      #select only alignments where both fwd and rev read match
      
      #NEW: allow more than one mapping, then select the best?
      my @considered = grep {$maps_count_fwd_1stread{$_} >= 1 and $maps_count_rev_2ndread{$_} >= 1} keys %maps_score;
      $isrc = 0 if (@considered);
      
      #NEW: treat reverse complement alignments
      if (scalar(@considered)==0 and $includes_rc_sequences) {
        @considered = grep {$maps_count_rev_1stread{$_} == 1 and $maps_count_fwd_2ndread{$_} == 1} keys %maps_score;
        $isrc = 1 if (@considered);
      }
      
      if (scalar(@considered) == 1) {
        if ($isrc) {
          #NEW: treat reverse complement alignments
          if (defined($RCdict{$considered[0]})) {
            $selectedmap = $RCdict{$considered[0]};
            $score = $maps_score{$considered[0]};
            #print("$counter RC: $selectedmap ***************************\n")

          } else {
            warn "Found reverse complement where there shouldn't be one";
            $selectedmap = "*";
            }
      ...
      %maps_score = ();
      %maps_count_fwd_1stread = ();
      %maps_count_fwd_2ndread = ();
      %maps_count_rev_1stread = ();
      %maps_count_rev_2ndread = ();
    ...
    if (defined($maps_score{$bamsplit[2]})) {
      ...
      if ($bits[4] and $bits[6]) {
        $maps_count_rev_1stread{$bamsplit[2]}++;
      } elsif ($bits[4] and $bits[7]) {
        $maps_count_rev_2ndread{$bamsplit[2]}++;
      } elsif ($bits[6]) {
        $maps_count_fwd_1stread{$bamsplit[2]}++;
      } else {
        $maps_count_fwd_2ndread{$bamsplit[2]}++;
      }
    } else {
      $maps_score{$bamsplit[2]} = cigar_to_score($bamsplit[5]);
      $maps_count_fwd_1stread{$bamsplit[2]} = 0;
      $maps_count_rev_1stread{$bamsplit[2]} = 0;
      $maps_count_fwd_2ndread{$bamsplit[2]} = 0;
      $maps_count_rev_2ndread{$bamsplit[2]} = 0;
      if ($bits[4] and $bits[6]) {
        $maps_count_rev_1stread{$bamsplit[2]} = 1;
      } elsif ($bits[4] and $bits[7]) {
        $maps_count_rev_2ndread{$bamsplit[2]} = 1;
      } elsif ($bits[6]) {
        $maps_count_fwd_1stread{$bamsplit[2]} = 1;
      } else {
        $maps_count_fwd_2ndread{$bamsplit[2]} = 1;
      }
    }
...
while (my ($barcode, $crs) = each (%BC_CRS_fixed)) {
    my @values = ($barcode, $crs->[0], $crs->[2], $crs->[3], 
                  sum(@{$crs->[1]}) / scalar(@{$crs->[1]}), 
                  min(@{$crs->[1]}), 
                  max(@{$crs->[1]}));
